(function (win, doc) {
	...
	*/
	...
	$start.addEventListener('click', startCounter, false);
	$stop.addEventListener('click', stopCounter, false);
	$reset.addEventListener('click', resetCounter, false);

	function startCounter() {
		$timer.value = +$timer.value + 1;
		counter = setTimeout(startCount, 1000);
	}
	...
	function resetCounter(argument) {
		$timer.value = 0;
		stopCounter();
	}
